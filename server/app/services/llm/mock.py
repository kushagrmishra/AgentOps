...
class MockProvider(LlmProvider):
    ...
    def _tool_args(self, tool: str, subject: str) -> dict:
        ...
        if tool == "write_file":
            return {"path": "output.txt", "content": "Sample output generated by agent"}
        if tool == "list_files":
            return {"path": ""}
    ...
    def _synthesise(self, system: str, task: str, messages: list[LlmMessage]) -> str:
        ...
        for result in results:
            ...
            if result.tool == "run_code":
                ...
                sections += [f"Wrote file: {result.body}", ""]
            elif result.tool == "list_files":
                sections += [f"Workspace files: {_short_goal(result.body, 100)}", ""]
